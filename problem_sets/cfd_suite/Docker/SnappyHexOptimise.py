...
class BasicDuctRun(InitialTEST):
    ...
    def Optimisationrun(self): #run simpleFoam
        ...
        current = os.getcwd()
        subprocess.call(['mpirun', '--allow-run-as-root','-np', str(self.ncores), self.solver4,'-parallel'], cwd=current+'/'+self.case_path, stdout=self.stdout)

        self.run_log(['mpirun', '--allow-run-as-root', '-np', str(self.ncores) , self.solver1 , '-parallel'], \
