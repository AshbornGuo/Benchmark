...
PATH_RESULT = PROJECT_ROOT / "results" / "TurbofanArch" / "MOEAD"
PATH_RESULT.mkdir(parents=True, exist_ok=True)

LOG_CSV = PATH_RESULT / f"TurbofanArch_MOEAD_seed{seed}.csv"

RUN_ROOT = PATH_RESULT / "runs" / "Turbofan"
...
if __name__ == "__main__":
    ...
    pd.DataFrame({"gen_time": gen_time}).to_csv(
        PATH_RESULT / f"TurbofanArch_MOEAD_gentime{seed}.csv",
        index=False,
        sep=";",
    )