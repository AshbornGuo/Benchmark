...
RESULT_ROOT = PROJECT_ROOT / "results" / "LayeredBeam" / "MOEAD"
RESULT_ROOT.mkdir(parents=True, exist_ok=True)

LOG_CSV = RESULT_ROOT / f"LayeredBeam_MOEAD_seed{seed}.csv"
...
def run_one_sim(sim_id: int, vector):
    ...
    try:
        ...
        out = f(vector, sim_id)  # mass, intrusion
...
if __name__ == "__main__":
    ...
    pd.DataFrame({"gen_time": gen_time}).to_csv(
        RESULT_ROOT / f"LayeredBeam_MOEAD_gentime{seed}.csv",
        index=False,
        sep=";",
    )