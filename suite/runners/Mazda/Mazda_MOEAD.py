...
LOG_CSV = os.path.join(PATH_RESULT, f"Mazda_MOEAD_seed{random_seed}.csv")
...
class ConstraintsAsPenaltyMOO(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        # 正确方式：让 evaluate 返回 dict
        _out = self.problem.evaluate(
            X,
            return_values_of=["F", "G"],          # 强制要到 F 和 G
            return_as_dictionary=True,
            **kwargs
        )

        F = _out["F"]                            # shape (n, n_obj)
        G = _out.get("G", None)                  # shape (n, n_constr) or None
        ...
        out["F"] = F + self.penalty * CV[:, None]   # (n,) -> (n,1) 广播到每个目标
...
# algorithm = MOEAD(pop_size=population_size) 
ref_dirs = get_reference_directions("das-dennis", 2, n_partitions=population_size-1)
