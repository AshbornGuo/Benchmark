...
# m_cv   = SingleTaskGP(train_X_n, train_cv)   # (n,1)
...
for i in range(T):
    ...
    # warm start
    # m_obj1.set_train_data(train_X_n, train_obj[:, 0].contiguous(), strict=False)  # (n,)
    # m_obj2.set_train_data(train_X_n, train_obj[:, 1].contiguous(), strict=False)  # (n,)
    # m_cv.set_train_data(train_X_n, train_cv.squeeze(-1).contiguous(), strict=False)  # (n,)
    ...
    # 2) Chebyshev scalarization（只对两个目标做标量化）
    scalarization = get_chebyshev_scalarization(weights=weights, Y=train_obj)

    # 3) MC objective: 输入 Z 是 (mc, ..., 3) -> 取前2个目标做 scalarization
    parego_objective = GenericMCObjective(lambda Z, X: scalarization(Z[..., :2]))

    # 4) best_f：建议只用“可行点”的 best（cv==0）
    train_y = torch.cat([train_obj, train_cv], dim=-1)  # (n, 3)
    FEAS_TOL = 1e-8
    is_feas = (train_cv.squeeze(-1) <= FEAS_TOL)
    if is_feas.any():
        best_f = parego_objective(train_y[is_feas]).max().item()
    else:
        # 如果一个可行点都没有，就退化用全体点（不完美，但不会崩）
        best_f = parego_objective(train_y).max().item()
