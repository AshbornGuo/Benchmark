...
K = 200  #别人连续优化的迭代次数最大才200，你离散每次都选512个点，这也太大了
T = 1000 - 50  
MC_SAMPLES = 16
BATCH_SIZE = 1
...
LOG_CSV = os.path.join(PATH_RESULT, f"Mazda_SBO_EHVI_seed{seed}.csv")
...
def calc_cv_from_con_raw(con_raw_list):
    ...
    # 另一种处理方式，只取范围最厉害的那个约束作为违反约束量
    # return torch.max(-c).item()
...
# 把初始参考点的数据转成tensor形式
train_X = torch.tensor(variables, dtype=torch.float32)                 # (n,222)
train_obj = torch.tensor(objs_max_init, dtype=torch.float32)      # (n,2)
train_cv  = torch.tensor(cvs_init, dtype=torch.float32)           # (n,1)
...
for i in range(T):
    ...
    m_cv   = SingleTaskGP(train_X_n, train_cv[:, 0:1].contiguous())  # (n,1)
