...
N_JOBS = 5
BATCH_SIZE = N_JOBS      # 每轮并行评估 5 个点
K_DISCRETE = 200         # BO 离散候选池大小
...
PATH_RESULT = r"C:/Users/guoji/Desktop/python3_11_test/results/mazda/SBO_EGBO_DISCRETE"

os.makedirs(PATH_RESULT, exist_ok=True)
LOG_CSV = os.path.join(PATH_RESULT, f"Mazda_EGBO_discrete_seed{seed}_256.csv")

# Per-eval isolated workdirs (parallel-safe)
RUN_ROOT = Path(PATH_RESULT) / "runs" / "MAZDA_EGBO_DISCRETE"
RUN_ROOT.mkdir(parents=True, exist_ok=True)

# Reference point in max-space
# Must be consistent with transform_obj_to_max_space()
...
def project_continuous_to_discrete(X_cont, dv):
    """
    Project each continuous candidate to nearest allowed discrete value.
    """
    out = []
    for row in X_cont:
        row_vars = []
...
def sample_discrete_candidate_tensor(dv, k):
    cand_list = [sample_random_vars(dv) for _ in range(k)]
    cand_list = unique_keep_order(cand_list)

    while len(cand_list) < k:
        cand_list += [sample_random_vars(dv) for _ in range(k)]
        cand_list = unique_keep_order(cand_list)

    cand_list = cand_list[:k]
    return torch.tensor(cand_list, dtype=torch.float32)


def build_model(train_X_n, train_obj, train_cv):
    """
    2 objectives + 1 aggregated CV model
    """
...
def egbo_generate_batch_discrete(
    acq,
    train_X,
    train_X_n,
    train_obj,
    ...
    """
    EGBO with discrete BO:
      1) BO candidate batch via optimize_acqf_discrete on random discrete pool
      2) EA candidate pool via NSGA-III in continuous normalized space
      3) project EA candidates to discrete space
      4) merge BO + EA discrete candidates
      5) rerank merged candidates by acquisition
      6) take top-q discrete points
    """

    # --------------------------
    # 1) BO discrete candidate batch
    # --------------------------
    X_cand = sample_discrete_candidate_tensor(dv, k_discrete)  # original discrete scale
    X_cand_n = norm_X(X_cand, lb, rng_)

    q_bo = min(q, X_cand_n.shape[0])

    X_bo_n, _ = optimize_acqf_discrete(
        acq_function=acq,
        choices=X_cand_n,
        q=q_bo,
    )

    # map selected normalized rows back to original discrete rows
    idxs_bo = torch.cdist(X_cand_n, X_bo_n).argmin(dim=0)
    X_bo = X_cand[idxs_bo]

    # --------------------------
    # 2) EA candidate pool (continuous normalized space)
    # --------------------------
    pareto_mask = is_non_dominated(train_obj)
    pareto_x = train_X_n[pareto_mask]
    pareto_y = -train_obj[pareto_mask]   # pymoo assumes minimization
    ...
    X_ea_n_cont = torch.tensor(newpop.get("X"), dtype=torch.float32)

    # --------------------------
    # 3) EA continuous -> original continuous -> nearest discrete
    # --------------------------
    X_ea_cont = denorm_X(X_ea_n_cont, lb, rng_)
    ea_vars_list = project_continuous_to_discrete(X_ea_cont, dv)
    ea_vars_list = unique_keep_order(ea_vars_list)

    X_ea = torch.tensor(ea_vars_list, dtype=torch.float32)
    X_ea_n = norm_X(X_ea, lb, rng_)

    # --------------------------
    # 4) Merge BO + EA discrete candidates
    # --------------------------
    merged_list = X_bo.detach().cpu().numpy().tolist() + X_ea.detach().cpu().numpy().tolist()
    merged_list = unique_keep_order(merged_list)

    existing_keys = set(tuple(map(float, row)) for row in train_X.detach().cpu().numpy().tolist())
    merged_list = [row for row in merged_list if tuple(map(float, row)) not in existing_keys]

    while len(merged_list) < q:
        extra = sample_random_vars(dv)
        key = tuple(map(float, extra))
        if key not in existing_keys and key not in [tuple(map(float, x)) for x in merged_list]:
            merged_list.append(extra)

    X_merge = torch.tensor(merged_list, dtype=torch.float32)
    X_merge_n = norm_X(X_merge, lb, rng_)

    # --------------------------
    # 5) Acquisition rerank
    # --------------------------
    scores = score_candidates_with_acq(acq, X_merge_n)
    idx = torch.argsort(scores, descending=True)

    X_best = X_merge[idx[:q]]
...
if __name__ == "__main__":
    ...
    while train_X.shape[0] < TOTAL_BUDGET:
        ...
        # qLogNEHVI with aggregated CV constraint
        acq = qLogNoisyExpectedHypervolumeImprovement(
            ...
            constraints=[lambda Z: Z[..., 2]],   # feasible if cv <= 0
        )
        ...
        # -------------------------
        # EGBO batch proposal (discrete BO + EA)
        # -------------------------
        X_next = egbo_generate_batch_discrete(
            acq=acq,
            train_X=train_X,
            train_X_n=train_X_n,
            train_obj=train_obj,
        ...
        vars_next_list = X_next.detach().cpu().numpy().tolist()
        vars_next_list = unique_keep_order(vars_next_list)

        # fill if anything goes wrong with duplicates
        existing_keys = set(tuple(map(float, row)) for row in train_X.detach().cpu().numpy().tolist())
        ...
        print(
            f"[EGBO-discrete] round={round_id}  "
