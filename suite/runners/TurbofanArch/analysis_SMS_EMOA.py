...
RESULT_DIR = PROJECT_ROOT / "results" / "TurbofanArch" / "SMS_EMOA"

PATH_SMSEMOA_331 = RESULT_DIR / "TurbofanArch_SMSEMOA_seed331.csv"
PATH_SMSEMOA_332 = RESULT_DIR / "TurbofanArch_SMSEMOA_seed332.csv"
PATH_SMSEMOA_333 = RESULT_DIR / "TurbofanArch_SMSEMOA_seed333.csv"
...
def read_res(path_result):
    ...
    F_all = np.vstack(objs_list.to_numpy())

    if F_all.shape[1] < 3:
        raise ValueError(f"{path_result} 的目标维度不足 3，实际是 {F_all.shape[1]}")
    ...
    finite_mask = finite_mask_nor & finite_mask_raw
...
cons_331, Fnor_331, Fraw_331 = read_res(PATH_SMSEMOA_331)
cons_332, Fnor_332, Fraw_332 = read_res(PATH_SMSEMOA_332)
cons_333, Fnor_333, Fraw_333 = read_res(PATH_SMSEMOA_333)
...
# =========================
plot_info = [
    (331, cons_331, Fraw_331),
    (332, cons_332, Fraw_332),
    (333, cons_333, Fraw_333),
]

for seed, cons, Fraw in plot_info:
    F_front0, F_dom, F_infeas = split_3obj_for_plot(Fraw, cons.to_numpy(dtype=bool))

    plot_3d_pareto_with_all_types(
        F_front0,
        F_dom,
        F_infeas,
        title=f"3D Pareto Front - SMSEMOA seed{seed}",
        save_path=RESULT_DIR / f"Pareto_front0_3D_alltypes_SMSEMOA_{seed}.png"
    )
