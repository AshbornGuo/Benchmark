...
# Continuous optimize_acqf_list settings
NUM_RESTARTS = 5
RAW_SAMPLES = 128
ACQ_MAXITER = 100

# Paths
PATH_RESULT = r"C:/Users/guoji/Desktop/python3_11_test/results/TurbofanArch/SBO_qLogNParEGO"
os.makedirs(PATH_RESULT, exist_ok=True)
LOG_CSV = os.path.join(PATH_RESULT, f"TurbofanArch_qLogNParEGO_seed{seed}.csv")
...
def transform_obj_to_max_space(obj_original):
    ...
    qLogNParEGO here works in max-space:
      y_obj = -F
    """
...
if __name__ == "__main__":
    ...
    while eval_id < TOTAL_BUDGET:
        ...
        # qLogNParEGO: one random scalarization per candidate in the batch
        acq_list = []
        for _ in range(q_now):
            w = sample_simplex(m, dtype=torch.double).squeeze(0)
            acq_list.append(
                qLogNParEGO(
                    model=model,
        ...
        print(
            f"[BO-Turbofan-qLogNParEGO] evals={eval_id}/{TOTAL_BUDGET} "
