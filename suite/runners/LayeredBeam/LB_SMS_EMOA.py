...
PATH_RESULT = r"C:/Users/guoji/Desktop/python3_11_test/results/LayeredBeam/SMSEMOA"
os.makedirs(PATH_RESULT, exist_ok=True)

LOG_CSV = os.path.join(PATH_RESULT, f"LayeredBeam_SMSEMOA_seed{seed}.csv")
...
def run_one_sim(sim_id: int, vector):
    ...
    try:
        ...
        out = f(vector, sim_id) #原始输出，求最小质量和最大能量
...
# MyCallback：每一代（或每次算法迭代）记录时间和写 CSV；
...
# LayeredBeamProblem：定义 pymoo 的“问题”
class LayeredBeamProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        objs, eval_time = run_one_sim(sim_id, x_list) # 这里还是原始输出
        mass = float(objs[0])
...
# main：清理 runs、初始化 CSV、并行 runner、运行 minimize
if __name__ == "__main__":
    ...
    pd.DataFrame({"gen_time": gen_time}).to_csv(
        os.path.join(PATH_RESULT, f"LayeredBeam_SMSEMOA_gentime{seed}.csv"),
        index=False,
        sep=";",
    )