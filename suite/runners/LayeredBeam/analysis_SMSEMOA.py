...
RESULT_DIR = PROJECT_ROOT / "results" / "LayeredBeam" / "SMSEMOA"




PATH_SMSEMOA_331 = RESULT_DIR / "LayeredBeam_SMSEMOA_seed331.csv"
PATH_SMSEMOA_332 = RESULT_DIR / "LayeredBeam_SMSEMOA_seed332.csv"
PATH_SMSEMOA_333 = RESULT_DIR / "LayeredBeam_SMSEMOA_seed333.csv"
...
Fnor_SMSEMOA_331 = read_res(PATH_SMSEMOA_331)
Fnor_SMSEMOA_332 = read_res(PATH_SMSEMOA_332)
Fnor_SMSEMOA_333 = read_res(PATH_SMSEMOA_333)
...
# # hypervolume
hv_SMSEMOA_331 = hv_analysis(Fnor_SMSEMOA_331, step)
hv_SMSEMOA_332 = hv_analysis(Fnor_SMSEMOA_332, step)
hv_SMSEMOA_333 = hv_analysis(Fnor_SMSEMOA_333, step)


# take average

hv_runs = [hv_SMSEMOA_331, hv_SMSEMOA_332, hv_SMSEMOA_333]   
...
plt.tight_layout()
plt.savefig("results/LayeredBeam/SMSEMOA/HV_curve.png", dpi=300, bbox_inches="tight")
plt.close()


############打印非支配层级每层有多少个点，但似乎现在结果显示这两个目标不冲突！！！！
##########
import numpy as np
from pymoo.util.nds.non_dominated_sorting import NonDominatedSorting

def count_front_layers(F, K=800):
    F = np.asarray(F)
    ...
    for layer in range(K):
        ...
        front_local = nds.do(F[remaining])[0]
        front_size = len(front_local)

        counts.append(front_size)
...
counts = count_front_layers(Fnor_SMSEMOA_331, K=800)

for i, c in enumerate(counts):
    print(f"front {i}: {c} points")
