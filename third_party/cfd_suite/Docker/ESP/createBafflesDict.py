...
## Generate an example dict without the need of specifying a string via
## command line
generate01 = False
if generate01:
    readSettings = "1"
    for i in range(24):
        readSettings = readSettings + "01"
else:
    readSettings = sys.argv[1]
...
def writeFromArray(name, val):
    createBafflesDictFile.write(name+"{type faceZone; zoneName "+name+"; patches{master{name "+name+"_m; type cyclic; neighbourPatch "+name+"_s; patchFields{p{type porousBafflePressure; patchType cyclic; D 0; I "+arrayBaffleSettings[val]+"; length 0.03; jump uniform 0;value uniform 0;}}}slave{name "+name+"_s; type cyclic; neighbourPatch "+name+"_m; patchFields{${...master.patchFields}}}}}\n")
...
for i in range(len(readSettings)):
    write(namesList[i],int(readSettings[i]))
